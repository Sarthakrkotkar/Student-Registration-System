...
public class MBA extends JInternalFrame implements ActionListener {
    ...
    public MBA(String rno, String title, int semno) {
    ...
    private void loadSubjectNames() {
        try {
            String sql = "Select SubjectName from subjectallocation where Semester=" + semester + " and Batch='MBA'";
            Connection con = Settings.getDBConnection();
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
