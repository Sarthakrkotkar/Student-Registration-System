...
public class MCA extends JInternalFrame implements ActionListener {
    ...
    public MCA(String rno, String title, int semno) {
    ...
    private void loadSubjectNames() {
        try {
            String sql = "Select SubjectName from subjectallocation where Semester=" + semester + " and Batch='MCA'";
            Connection con = Settings.getDBConnection();
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
